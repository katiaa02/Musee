            ...
            while ($row = $result_get_favoris->fetch_assoc()) {
                ...
                if ($response === false) {
                    ...
                    if (isset($data['records']) && count($data['records']) > 0) {
                        ...
                        echo '<div class="card">';
                        echo '<div class="card-body">';
                        echo '<a href="description.php?nomoff=' . urlencode($museumName) . '">' . $museumName . '</a>';
                        if (isset($museumDetails['adrl1_m'])) {
                            echo '<p class="card-text"><strong>Adresse:</strong> ' . $museumDetails['adrl1_m']  . '</p>'; 
                        } else {
                            echo '<p class="card-text"><strong>Ville:</strong> ' . $museumDetails['ville_m']  . '</p>'; 
                        }
